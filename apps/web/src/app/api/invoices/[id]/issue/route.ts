...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('invoices').update({ status: 'issued', issue_date: new Date().toISOString().slice(0, 10), due_date: dueDate }).eq('id', id).eq('user_id', session.user.id).eq('status', 'draft').select().single()
  if (error || !data) return NextResponse.json({ error: 'No se pudo emitir la factura' }, { status: 400 })
